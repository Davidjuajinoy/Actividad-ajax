...
class Usuario extends DB{
  ...
  public function searchM($nombre){
   ...
   public function detalles($id){
      try{
        $stm=parent::connect()->prepare("SELECT * FROM usuarios WHERE id = $id");
        $stm->execute();
      ...
      try{
        $stm=parent::connect()->prepare("UPDATE usuarios SET nombres = '$nombres', `apellidos` = '$apellidos', `correo` = '$correo', `direccion` = '$direccion', `telefono` = '$telefono', `foto_perfil` = '$foto_perfil' WHERE `usuarios`.`id` = '$id'");
        $stm->execute();
        return $stm->fetch(PDO::FETCH_OBJ);
